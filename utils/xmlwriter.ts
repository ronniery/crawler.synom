...
export class XmlWriter {
    ...
    /**
     * Tell if the generator should use terms
     * 
     * @private
     * @type {boolean}
     * @memberOf XmlWriter
     */
    private useTerms: boolean;

    /**
     * Tell if the generator should use words
     * 
     * @private
     * @type {boolean}
     * @memberOf XmlWriter
    ...
    /**
     * When the words and terms are full loaded to generate the xml
     * the core function will use this to skipp all words into {Scope.senses}
     * that has spaces if this property has 'true' value
     * 
     * @private
     * @type {boolean}
     * @memberOf XmlWriter
     */
    private skipSpacedWords: boolean;

    /**
     * Set the maximum level of iterations over {Scope.senses}
     * 
     * @private
     * @type {number}
     * @memberOf XmlWriter
     */
    private untilWhichLevel: number;

    /**
     * Db reference to read/write in it
     * 
     * @private
     ...
     * @memberOf XmlWriter
     */
    constructor(options: WriterOptions = new WriterOptions()) {
        this.useTerms = options.useTerms,
            this.useWords = options.useWords,
            this.skipSpacedWords = options.skipSpacedWords,
            this.untilWhichLevel = options.untilWhichLevel,
     ...
     * @memberOf XmlWriter
     */
    public generate(xmlPath: string = "./thesaurus.xml"): void {
        if (!this.useTerms && !this.useWords) {
            console.error("Haha... We have a clown here!");
        }

        var localMaxLevel = this.untilWhichLevel;
        var self = this;
